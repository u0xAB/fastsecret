...
import fscore.UpdateModelListener;
 ...
 */
public class ModelListener implements UpdateModelListener {
    RecordTableModel model;
    ModelListener(RecordTableModel rtm)
    {
        model = rtm;
    }
    @Override
    public void updateModel() {
        model.clear(); 
        Storage.INSTANCE.fileMap().stream().forEach((ri) -> {
            if (ri.date != null) model.addRecord(ri);
        });
